...
export async function getScreenData(): Promise<ScreenData> {
    const code = "return await getScreenData();";
    const data = await codeExec(code);
    const buffer = Buffer.from(JSON.parse(data.fileBuffer));
    return { screen: data.screen, fileBuffer: buffer } as ScreenData;
}

// Gets the current active window data, buffer needs to be parsed with JSON.parse
export async function getActiveWindowData(): Promise<ScreenData> {
    const code = "return await getActiveWindowData();";
    const data = await codeExec(code);
    const buffer = Buffer.from(JSON.parse(data.fileBuffer));
    return { screen: data.screen, fileBuffer: buffer } as ScreenData;
}
...
export async function pushResultStream(token: string): Promise<void> {
    const code = `await pushResultStream(\`${token}\`);`;
    await codeExec(code);
}
...
export interface ScreenData {
    screen: { id: string, name: string, display_id: string };
    fileBuffer: Buffer;
}
